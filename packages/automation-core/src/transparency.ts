...
export function verifyAICTransparencyIndex(input: {
  ...
  if (!key || key.issuer_id !== index.log_id || key.status !== "active") {
    ...
    try { derived = keyId(key.public_key_pem); } catch { derived = undefined; }
    ...
    if (derived !== key.key_id || !inWindow) {
      checkpointFailed = true;
      finding(findings, "checkpoint_signer_untrusted", "Pinned log key material or validity window does not authorize the checkpoint.");
    } else {
      try {
        if (!verify(null, domainBytes(CHECKPOINT_DOMAIN, statement), createPublicKey(key.public_key_pem), Buffer.from(checkpoint.signature.value, "base64"))) {
          checkpointFailed = true;
