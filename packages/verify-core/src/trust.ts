/** Minimal cryptographic trust primitives for AIC verification consumers. */
import {
  createHash,
  createPrivateKey,
  createPublicKey,
...
function canonicalize(value: unknown, ancestors = new Set<object>()): unknown {
  ...
  if (ancestors.has(value)) {
    throw new TypeError("AIC canonical JSON cannot contain cycles.");
  }
...
function createKeyIdFromKey(publicKey: KeyObject): string {
  const der = publicKey.export({ format: "der", type: "spki" });
  return `sha256:${createHash("sha256").update(der).digest("hex")}`;
}

function createKeyId(publicKeyPem: string): string {
  return createKeyIdFromKey(createPublicKey(publicKeyPem));
}
  ...
  if (!key) {
    ...
    if (!publicKey || publicKey.asymmetricKeyType !== "ed25519") {
      ...
      addFinding(
        findings,
        "key_type_mismatch",
        "The trusted key material must be an Ed25519 public key."
      );
    }
    const derivedKeyId = publicKey ? createKeyIdFromKey(publicKey) : undefined;
    ...
    if (!publicKey || publicKey.asymmetricKeyType !== "ed25519") {
      ...
      addFinding(
        findings,
        "signature_invalid",
        "The signature cannot be verified with non-Ed25519 key material."
      );
    } else try {
  ...
  if (input.proof !== undefined) {
    try {
      ...
      const matches =
        ...
        statement.subject.generated_at === proof.generated_at &&
        statement.subject.evidence_level === proof.evidence_level;
