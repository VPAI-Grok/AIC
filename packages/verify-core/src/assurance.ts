/** Deterministic behavior-proof regeneration for AIC verification consumers. */
import {
...
function parityValue(observation: AICBehaviorObservation): unknown {
  return {
    ...(observation.confirmation === undefined ? {} : { confirmation: observation.confirmation }),
    ...(observation.error_code === undefined ? {} : { error_code: observation.error_code }),
...
export function verifyAICBehavior(input: AICBehaviorVerificationInput): AICBehaviorProof {
  ...
  const generatedAtIsValid =
    requestedGeneratedAt === undefined || isAICRfc3339DateTime(requestedGeneratedAt);
    ...
    const surfaceResults = scenario.surfaces.map((surfaceId) => {
      ...
      if (!observation) {
        ...
        if (
          ...
          createAICCanonicalJson(observation.outcome) !== createAICCanonicalJson(scenario.expected.outcome)
        ) {
          addFinding(
            findings,
            "outcome_mismatch",
      ...
      return {
        ...
        ...(observation === undefined ? {} : { observation_mode: observation.mode }),
    ...
    if (scenario.parity === "required") {
      ...
      const matches =
        ...
        scenarioObservations.every(
          (observation) =>
            createAICCanonicalJson(parityValue(observation)) ===
            createAICCanonicalJson(parityValue(reference))
        );
