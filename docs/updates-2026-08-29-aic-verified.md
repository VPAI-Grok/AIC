...
The checkout reference runs success, authorization denial, confirmation decline, business failure, and safe recovery through both the rendered human UI and Chrome's real `document.modelContext`. Ten observations pass with zero findings. Each run records browser/API compatibility, application state, charge count, and a screenshot digest.
...
The important boundary is explicit: a valid signature proves that a trusted issuer signed the exact claim. It does not independently prove that production is live or certified. The remaining market gate is genuine external adoption and evidence from independently operated remote runners—not another WebMCP wrapper.
