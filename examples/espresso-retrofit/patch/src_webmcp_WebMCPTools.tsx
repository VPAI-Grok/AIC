...
// The AIC hook reports a lifecycle status; the badge below expects the
// {supported, registered} shape the author's hook returns. Map between them so
// the existing UI keeps working unchanged.
function asToolResult(status: string) {
  return {
...
export function WebMCPTools() {
  ...
  const results = [
    ...
    asToolResult(
      useAICWebMCPTool<{ code: string }, Awaited<ReturnType<typeof applyCoupon>>>(
        () => ({
          action: APPLY_COUPON_ACTION,
          element: APPLY_COUPON_ELEMENT,
          // Same domain operation the human coupon form calls.
          execute: (args) => applyCoupon(args),
          registry,
          tool: {
            annotations: mutating,
            description:
          ...
          validate: async (args) => {
            if (!args?.code?.trim()) throw new Error("A coupon code is required.");
          },
          // applyCoupon returns the rebuilt cart, so the claim is checkable.
          verify: async (cart) =>
            cart?.coupon?.code?.toUpperCase() != null,
        }),
        [registry],
        { enabled: loggedIn },
      ).status,
    ...
    asToolResult(
      useAICWebMCPTool<Record<string, never>, {
        effect: string;
        orderId: number;
        totalCents: number;
      }>(
        () => ({
          ...
          // Critical risk requires authorization. Wasp enforces ownership
          // server-side in placeOrder via requireUser; this is the client-side
          // precondition an agent can be refused on before anything is charged.
          authorize: async () => loggedIn,
          confirm: async (request) => window.confirm(request.prompt),
          element: CHECKOUT_ELEMENT,
          // Same domain operation the human Checkout button calls.
          execute: async () => {
          ...
          tool: {
            annotations: mutating,
            description:
              "Place the order for everything in the cart. Requires human confirmation; irreversible once confirmed.",
            inputSchema: emptySchema,
            name: "checkout",
          },
          validate: async (input) => {
            if (Object.keys(input ?? {}).length > 0) {
              throw new Error("checkout does not accept input fields.");
            }
          },
          // placeOrder returns the new order id and empties the cart, so the
          // declared postcondition is verifiable rather than aspirational.
          verify: async (result) =>
            Number.isInteger(result?.orderId) && result.orderId > 0,
        }),
        [registry, loggedIn],
        { enabled: loggedIn },
      ).status,
