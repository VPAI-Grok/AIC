...
## 🤖 Simulating AI Interoperability via MCP

To test how an autonomous agent fetches instructions and interactive boundaries via raw Model Context Protocol (MCP), a testing script is provided. Keep the dev server running, and in another terminal run:

```bash
node simulate-mcp-client.mjs
```

This establishes a local JSON-RPC connection to `@aicorg/mcp-server` via `stdio`, mimicking how local AI like Claude or Cursor queries the active UI. The parsed payload generated by `@aicorg/mcp-server` matching available page elements will be safely written to `mcp-simulation-result.json`.
